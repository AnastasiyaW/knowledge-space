...
**Last event:** 2023-06-02 · 1 dated since 2023-06-02 · **Researched:** 2026-09-04 · confidence: medium

## What it is

GenMM — a SIGGRAPH 2023 research codebase for technical animators who need new skeletal-motion sequences from one or a few example clips, not a text-to-motion model.

- Synthesizes diverse variants, completes motion, follows keyframe poses, creates loops, reassembles motion across heterogeneous skeletons, and changes locomotion trajectories.
- The documented local stack is Python 3.8, PyTorch 1.12.1 and CUDA 11.3; the authors report about 0.05 s per motion on a Tesla V100 and 0.2 s on an Apple M1.

## Development line

- **2023-06-02 — GenMM public project materials linked.** On 2023-06-02, the project published its project page, source repository, and demo link. This milestone opened evaluation entry points for the project, though the unavailable source text leaves the exact release claim and technical details unestablished.

## What changed

2023-06-02 — GenMM went live with a project page, source repository, and browser demo as an example-based, training-free motion-synthesis system. The page documents motion completion, keyframe guidance, looping, reassembly, and locomotion. 2023-07-13 — the entry provides only the existing browser-demo endpoint. It identifies no new model, package version, or feature, so this is not a confirmed software release. 2026-09-04 — the original GitHub URL redirects to liyuuuu98/GenMM; the current releases page exposes a single Blender add-on pre-release, v0.0.1. The retrieved release page gives only "29 Jul 12:48", not a year, so no full release date is assigned.

## How to use this

From 2023-06-02, we could use the public GenMM project page, source repository, and demo as entry points for evaluation; the 2023-07-13 demo link alone does not establish a further practical change.
...
- 2023-06-02 — the README release checklist is obsolete as an availability statement: the repository now contains the local runner and GitHub lists the Blender add-on v0.0.1 pre-release.
...
- The 2023-07-13 entry supplies only the existing demo URL. Without its source text, we cannot attribute any separate capability or version to that date.

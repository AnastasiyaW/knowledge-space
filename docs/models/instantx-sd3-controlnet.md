...
InstantX SD3 ControlNet provides control-image adapters for Stable Diffusion 3.

- Canny: follows edge maps.
- Pose: follows human pose.
- Tile: follows image guidance.
- Depth: follows depth maps.

Canny and Pose are 0.6B F16 checkpoints trained at 1024×1024 only.

We use them when gated SD3 Medium and a 1024-square workflow fit. Compatibility with later SD3 models is not established here.

## Development line

- **2024-06-15 — InstantX SD3 ControlNet Canny repositories appeared on Hugging Face.** Hugging Face links pointed to a Canny_alpha_512 repository and a Canny repository on 2024-06-15. This made Canny-conditioned SD3 ControlNet artifacts available in this project line.
- **2024-06-17 — InstantX SD3 ControlNet Pose repository appeared on Hugging Face.** A Hugging Face link pointed to the SD3-Controlnet-Pose repository on 2024-06-17. This added a pose-conditioned artifact to the project line.

## What changed

- 2024-06-15 — Canny received its configuration and checkpoint. The separate `Canny_alpha_512` endpoint remains unverified today.
- 2024-06-17 — Pose documentation and assets changed, and Tile received its checkpoint. Pose's original checkpoint had landed on 15 June.
...
We can use InstantX SD3 ControlNet Canny artifacts for edge-guided workflows starting 2024-06-15, and the pose-conditioned artifact starting 2024-06-17. Check compatibility, licensing, revision, and workflow requirements before use.
...
- Start Canny or Pose at 1024×1024; their cards say other resolutions can be suboptimal because training used 1024×1024 only.
  — <https://huggingface.co/InstantX/SD3-Controlnet-Canny>
- Start with the author demo's `controlnet_conditioning_scale=0.5`, then tune deliberately for each control image rather than treating it as universal.
  — <https://huggingface.co/InstantX/SD3-Controlnet-Pose>
- Use the documented SD3 Medium base for a known-compatible recipe; do not assume these checkpoints work unchanged with another SD3-family base.
  — <https://huggingface.co/InstantX/SD3-Controlnet-Canny>
- A June 2024 Chinese ComfyUI hands-on report advised low weights, early control end, and treating results as a starting image. Keep that as a historical workflow observation, not a current universal range.
  — <https://www.youtube.com/watch?v=5SdRiZ53WPM>

## Superseded by this

- 2024-06-19 — The Diffusers requirement replaced the old installation path that cloned `instantX-research/diffusers_sd3_control` branch `sd3_control`. Source: https://huggingface.co/InstantX/SD3-Controlnet-Canny/commit/a59221661ea027c11d813cbc595133ca8e56adb5
...
- https://huggingface.co/InstantX/SD3-Controlnet-Canny_alpha_512 was not retrievable; its exact weights, resolution, and relationship to Canny remain unverified.
- The 2024-07-03 Depth update was only an upload of two files rather than a named release, so its functional scope is unknown.
- No first-party compatibility matrix exists for Canny, Pose, Tile, and Depth checkpoints with SD3.5, current ComfyUI versions, or modern consumer-GPU VRAM.
- No explicit licence is verified from the Canny, Pose, or Tile cards; do not infer their terms from the Apache-2.0 label on Depth.
...
- **Practical note:** From 2024-06-15, we can use InstantX SD3 ControlNet Canny artifacts for edge-guided workflows; from 2024-06-17, we can use the pose-conditioned artifact for pose-guided workflows. Verify compatibility, licensing, revision, and workflow requirements before use.
